import rclpy
from rclpy.node import Node
import math
from geometry_msgs.msg import Twist
from turtlesim.msg import Pose
import sys

class GoToGoal(Node):
    def __init__(self):
        super().__init__("goal_node")
        self.publisher = self.create_publisher(Twist, '/turtle1/cmd_vel', 10)
        self.subscriber = self.create_subscription(Pose, '/turtle1/pose', self.pose_callback, 10)
        self.timer = self.create_timer(0.1, self.go_to_goal)
        self.pose = None  
    def pose_callback(self, data):
        self.pose = data  

    def go_to_goal(self):
        goal = Pose()
        goal.x = float(sys.argv[1])
        goal.y = float(sys.argv[2])
        #goal.theta = float(sys.argv[3])
        new_vel=Twist()
        
        distance = math.sqrt((goal.x - self.pose.x)**2 + (goal.y - self.pose.y)**2)
        theta = math.atan2((goal.y - self.pose.y), (goal.x - self.pose.x))
        distance_tol=0.3
        angular_tol=0.1
        angle_error=theta-self.pose.theta
        if abs(angle_error)>angular_tol:
            new_vel.angular.z=angle_error
            new_vel.linear.x=0.0
        else:
            if distance>distance_tol:
                new_vel.linear.x=distance
                new_vel.angular.z=0.0
            else:
                new_vel.linear.x=0.0
                new_vel.angular.z=0.0
                log_msg2='goal reached'
                self.get_logger().info(log_msg2)   
                quit()
        log_msg = 'DTG: {:.3f}, ATG: {:.3f}'.format(distance, theta)
        self.get_logger().info(log_msg)

        
       
        self.publisher.publish(new_vel)


def main(args=None):
    rclpy.init(args=args)
    node = GoToGoal()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()


if __name__ == '__main__':
    main()
